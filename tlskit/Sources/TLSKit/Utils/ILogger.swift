...
internal func logOpenSSLError(inFile: String, atLine: Int) {
    var opensslFile: UnsafePointer<CChar>? = nil
    var opensslLine: Int32 = -1
    ERR_peek_last_error_line(&opensslFile, &opensslLine)
    if opensslFile == nil {
        printError("[\(inFile):\(atLine)] Unsuccessful OpenSSL operation but no value returned from ERR_peek_last_error_line")
    } else {
        let fileName = String(cString: opensslFile!)
        printError("[\(inFile):\(atLine)] OpenSSL error in \(fileName):\(opensslLine)")
    }
}
