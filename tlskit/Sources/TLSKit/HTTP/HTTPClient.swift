...
fileprivate let maxHeaderSize = 102400 /* 100KiB - same as libcurl */

/// The embedded DNSKit HTTP client.
public final class HTTPClient: Sendable {
    // Nonisolated because the network framework only calls one callback at a time
    nonisolated(unsafe) fileprivate var headerData = Data()

    internal func requestFor(host: String) -> Data {
        let userAgent = "TLSKit" + (UserAgentSuffix != nil ? " \(UserAgentSuffix!)" : "")
        let request = "GET / HTTP/1.1\r\nHost: \(host)\r\nUser-Agent: \(userAgent)\r\nAccept: */*\r\n\r\n"
        printDebug("[\(#fileID):\(#line)] HTTP Request: \(request)")
        return request.data(using: .ascii)!
    }

    fileprivate func connectionReadLoop(_ connection: NWConnection, _ statusCode: UInt16, _ complete: @Sendable @escaping (Result<HTTPServerInfo, Error>) -> Void) {
        connection.receive(minimumIncompleteLength: 1, maximumLength: 1024) { oContent, _, _, oError in
            ...
            guard let content = oContent else {
                ...
                complete(.failure(MakeError("No response")))
                return
            }
            ...
            if hasAllHeaders {
                ...
                complete(.failure(MakeError("HTTP header data exceeded maximum length")))
                return
            } else {
    ...
    internal func response(from connection: NWConnection, _ complete: @Sendable @escaping (Result<HTTPServerInfo, Error>) -> Void) {
        ...
        connection.receive(minimumIncompleteLength: 12, maximumLength: 12) { oContent, _, _, oError in
            ...
            if httpVersion != "http/1.1" {
                ...
                complete(.failure(MakeError("Unsupported HTTP version")))
                return
            }
            ...
            guard let statusCode = UInt16(statusCodeString) else {
                ...
                complete(.failure(MakeError("Invalid HTTP status code \(statusCodeString)")))
                return
            }
            if statusCode < 100 || statusCode > 599 {
                printError("[\(#fileID):\(#line)] Invalid HTTP status code: \(statusCode)")
                complete(.failure(MakeError("Invalid HTTP status code \(statusCode)")))
                return
            }
    ...
    internal func response(from bio: OpaquePointer) -> Result<HTTPServerInfo, Error> {
        ...
        var read = responseGreeting.withUnsafeMutableBytes {
            ...
            return BIO_read(bio, $0.baseAddress, 12)
        }
        if read != 12 {
            printError("[\(#fileID):\(#line)] No response from HTTP server")
            return .failure(MakeError("No response"))
        }
        ...
        if httpVersion != "http/1.1" {
            printError("[\(#fileID):\(#line)] Unknown or unsupported HTTP version in response: \(httpVersion)")
            return .failure(MakeError("Unsupported HTTP version"))
        }

        let statusCodeString = String(decoding: responseGreeting[9..<12], as: UTF8.self)
        guard let statusCode = UInt16(statusCodeString) else {
            printError("[\(#fileID):\(#line)] Invalid HTTP status code: \(statusCodeString)")
            return .failure(MakeError("Invalid HTTP status code \(statusCodeString)"))
        }
        if statusCode < 100 || statusCode > 599 {
            printError("[\(#fileID):\(#line)] Invalid HTTP status code: \(statusCode)")
            return .failure(MakeError("Invalid HTTP status code \(statusCode)"))
        }
        ...
        while !hasAllHeaders {
            ...
            read = headerBuffer.withUnsafeMutableBytes {
                ...
                return BIO_read(bio, $0.baseAddress, 1024)
            }
            if read == 0 {
                printError("[\(#fileID):\(#line)] No response from HTTP server")
                return .failure(MakeError("No response"))
            ...
            if headerData.count + headerBuffer.count > maxHeaderSize {
                printError("[\(#fileID):\(#line)] HTTP header data exceeded maximum length")
                return .failure(MakeError("HTTP header data exceeded maximum length"))
            }
            ...
            headerData.append(Data(headerBuffer))
        }

        self.headerData = Data(headerData[0..<headersEndIdx])
        let headers = HTTPHeaders.fromResponse(self.headerData)
        let serverInfo = HTTPServerInfo(headers: headers, statusCode: statusCode)
