...
LATEST_VERSION=$(curl -Ss https://api.tlsinspector.com/rootca/latest | jq -r .version)

if [[ $CURRENT_VERSION == $LATEST_VERSION ]]; then
    exit 0
fi

curl -Ss https://api.tlsinspector.com/rootca/metadata/${LATEST_VERSION} > bundle_metadata.json
curl -OSs https://api.tlsinspector.com/rootca/asset/${LATEST_VERSION}/bundle_metadata.json.sig
...
function download_bundle {
    ...
    curl -OSs https://api.tlsinspector.com/rootca/asset/${LATEST_VERSION}/${NAME}_ca_bundle.pem
    curl -OSs https://api.tlsinspector.com/rootca/asset/${LATEST_VERSION}/${NAME}_ca_bundle.pem.sig
